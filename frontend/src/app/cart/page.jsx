...
export default function CartPage() {
  ...
  function handleCheckout() { router.push(user ? "/checkout" : "/login?redirect=/checkout"); }
  ...
  if (items.length === 0) {
    return (
      <div className="cart-empty container" data-testid="cart-empty">
        ...
        <Link href="/shop" className="btn" data-testid="cart-empty-cta">Start Shopping</Link>
      </div>
    );
  }
    ...
    <div className="cart-page">
      <div className="container">
        <div className="cart-heading">
          <div>
            <span className="eyebrow">Your Bag</span>
            <h1 data-testid="cart-heading">{totalItems} <em>{totalItems === 1 ? "item" : "items"}</em></h1>
          </div>
          <Link href="/shop" className="link-arrow" data-testid="cart-continue-shopping"><span className="arrow">←</span> Continue shopping</Link>
        </div>
        ...
        <div className="cart-layout">
          <div className="cart-items" data-testid="cart-items">
            {items.map((item) => (
              <div className="cart-item" key={item.id} data-testid={`cart-item-${item.id}`}>
                <Link href={`/product/${item.id}`} className="cart-item-media"><img src={item.image} alt={item.name} /></Link>
                <div className="cart-item-info">
                  ...
                  <div className="cart-item-row">
                    ...
                    <button className="cart-remove" onClick={() => removeFromCart(item.id)} data-testid={`cart-remove-${item.id}`}>Remove</button>
                  </div>
                </div>
          ...
          <div className="cart-summary" data-testid="cart-summary">
            ...
            {shipping > 0 && <p className="summary-note">Add ₹{(999 - totalPrice).toLocaleString("en-IN")} more for free shipping.</p>}
            <div className="summary-row summary-total"><span>Total</span><span>₹{grandTotal.toLocaleString("en-IN")}</span></div>
            <button className="btn btn-block" onClick={handleCheckout} data-testid="cart-checkout-btn">Proceed to Checkout</button>
